...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # edits the previous screen in place if the user re-runs /start, or
    # sends a new message on their very first visit.
    ...
    try:
        ...
        if result.startswith("ACCESS_NUMBER:"):
            ...
            if len(parts) >= 3:
                ...
                # Edit the screen in place, then anchor the 20-minute window
                # to the timestamp Telegram's own servers stamped on this
                # message — not the local system clock, which can drift.
                sent = await render(
                    context, chat_id,
                    "✅ *NUMBER PURCHASED*\n\n"
                    f"📱 Number: `{number}`\n\n"
...
async def get_number(context, chat_id, query=None):
    # Original purchase always starts the cascade at the first operator (Airtel).
    await purchase_number(context, chat_id, operator=OPERATOR_SEQUENCE[0], require_payment=True, query=query)
