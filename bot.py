    ...
    try:
        ...
        if result.startswith("ACCESS_NUMBER:"):
            ...
            if len(parts) >= 3:
                ...
                # Silently watch for the code in the background — the user
                # only gets DM'd once it actually arrives (or on expiry).
                ...
                await message.reply_text(
                    ...
                    "🔔 We'll DM you automatically the moment your code arrives "
                    "— no need to keep checking.\n\n"
                    "👉 *Impatient? Tap CHECK STATUS anytime* to check right now. "
                    "If nothing shows up after 3.5 minutes, tap 🔴 HELP for a free "
                    "replacement number.",
                    reply_markup=InlineKeyboardMarkup(keyboard),
                    parse_mode="Markdown",
                )
...
async def handle_status_result(send, context, result, notify_waiting=True):
    """
    ...
    notify_waiting controls whether "still waiting" messages get sent. The
    background poller passes False so it stays silent and only DMs the user
    once a real code shows up — manual CHECK STATUS always passes True
    (default) so a tap always gets a reply. Requesting a replacement number
    is handled separately via the HELP button, not from here.
    ...
    if not notify_waiting:
        # "other"/"empty" outcomes during a silent poll — stay quiet, try again next loop.
        return False
...
async def poll_for_code(bot, chat_id, context, activation_id, expiry_time):
    ...
    periodically but stays completely silent — the user is only DM'd once a
    real code arrives, or if the number expires/gets cancelled. This is what
    keeps the auto-poller from spamming the customer with "still waiting"
    messages; manual CHECK STATUS is still there for anyone who wants a
    right-now update.
    """
    ...
    while True:
        ...
        try:
            ...
            # Transient network hiccup — just try again next loop, no need to alert the user.
            continue

        should_stop = await handle_status_result(send, context, result, notify_waiting=False)
        if should_stop:
            return
