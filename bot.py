        ...
        if result.startswith("ACCESS_NUMBER:"):
            ...
            if len(parts) >= 3:
                ...
                await message.reply_text(
                    ...
                    "instead of waiting for the auto-notification.",
                    reply_markup=InlineKeyboardMarkup(keyboard),
                    parse_mode="Markdown",
                )
...
async def handle_status_result(send, context, result, notify_waiting=True):
    ...
    notify_waiting controls whether "still waiting" / replacement-offer
    messages get sent. The background poller passes False so it stays
    silent and only DMs the user once a real code shows up — manual
    CHECK STATUS always passes True (default) so a tap always gets a reply.
    ...
    if kind == "waiting":
        ...
        if not notify_waiting:
            # Silent background poll — just track attempts, don't DM.
            return False
