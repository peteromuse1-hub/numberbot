...
COUNTRY = 8
SERVICE = "vi"
MAX_PRICE = 0.03  # Maximum provider price we are willing to pay
...
async def purchase_number(context, chat_id, operator=None, require_payment=True, query=None, force_new_on_success=False):
    ...
    try:
        ...
        if result.startswith("ACCESS_NUMBER:"):
            ...
            # our strict MAX_PRICE ceiling. Do NOT silently raise the ceiling.
            minimum = result.split(":", 1)[1].strip()
            await render(
                context, chat_id,
                "❌ No Viber Kenya number is currently available within "
                f"the KSh-equivalent price limit we set (provider ceiling: ${MAX_PRICE:.3f}).\n\n"
                f"HeroSMS currently requires at least ${minimum}. "
                "The bot will not buy above your ceiling.",
                query=query,
                dead_end=True,
            )
