...
# Fallback ceiling, only used if the live price lookup below fails for any
# reason (network hiccup, HeroSMS format change, etc).
MAX_PRICE = 0.03

# Hard safety cap: no matter what the live price lookup returns, we will
# never offer more than this. Protects you if HeroSMS prices spike or the
# lookup misbehaves. Tune this to whatever the worst-case price you'd still
# be comfortable paying is.
ABSOLUTE_MAX_PRICE = 0.06

# Extra cushion added on top of the current lowest live price, so we don't
# get rejected the instant the price ticks up by a fraction of a cent
# between our lookup and the actual purchase.
PRICE_BUFFER = 0.005
...
# Your personal Telegram numeric user ID (not username) — only this ID may
# run /broadcast. Get yours by messaging @userinfobot on Telegram.
# Left unset, /broadcast quietly does nothing for everyone, including you.
...
AUTO_CHECK_INTERVAL = 3  # seconds between silent background code checks


def _autocode_job_name(chat_id):
    return f"autocode_{chat_id}"


def _cancel_autocode_job_app(application, chat_id):
    """Stops the silent background code watcher for this chat, if any."""
    for job in application.job_queue.get_jobs_by_name(_autocode_job_name(chat_id)):
        job.schedule_removal()


def _cancel_autocode_job(context, chat_id):
    _cancel_autocode_job_app(context.application, chat_id)


def _schedule_autocode_job_app(application, chat_id, activation_id):
    """
    Starts (or restarts) a silent background watcher that checks HeroSMS
    every few seconds for this number's code. It sends NOTHING while the
    code hasn't arrived — one single message goes out once it does.
    Assumes a private 1:1 chat, where chat_id doubles as user_id.
    """
    _cancel_autocode_job_app(application, chat_id)
    application.job_queue.run_repeating(
        auto_check_code,
        interval=AUTO_CHECK_INTERVAL,
        first=AUTO_CHECK_INTERVAL,
        chat_id=chat_id,
        user_id=chat_id,
        name=_autocode_job_name(chat_id),
        data={"activation_id": activation_id},
    )


def _schedule_autocode_job(context, chat_id, activation_id):
    _schedule_autocode_job_app(context.application, chat_id, activation_id)


async def auto_check_code(context: ContextTypes.DEFAULT_TYPE):
    """
    Runs every AUTO_CHECK_INTERVAL seconds for an active number. Stays
    completely silent until the code arrives, then sends ONE message with the
    code and stops. Also stops quietly if the number was replaced, the code was
    already fetched manually, the number was cancelled, or the 20 minutes ran out.
    """
    job = context.job
    chat_id = job.chat_id
    ...
    if ud.get("activation_id") != activation_id or ud.get("code_received"):
        job.schedule_removal()
        return

    expiry_time = ud.get("expiry_time")
    ...
    try:
        ...
        return  # Network hiccup — stay silent and try again next tick.

    if kind == "code":
        # Re-check: the user may have fetched it manually while we waited.
        if ud.get("code_received") or ud.get("activation_id") != activation_id:
            job.schedule_removal()
            return
        ud["code_received"] = True
        job.schedule_removal()
        await render(
            context, chat_id,
            f"✅ Your code is: `{text}`",
...
FUN_FACTS = [
    "🎲 The first SMS ever sent said \"Merry Christmas\" — back in 1992.",
    "🎲 Kenya's M-Pesa processes more transactions per day than most countries' entire banking systems combined.",
    "🎲 A phone number can technically have up to 15 digits under the international standard — Kenyan numbers just don't need that many.",
    "🎲 The '@' symbol in emails predates computers — it was originally an accounting shorthand for \"at the rate of.\"",
    "🎲 Telegram was built by two brothers who also created Russia's biggest social network, VK.",
    "🎲 The word \"robot\" comes from the Czech word \"robota,\" meaning forced labor.",
    "🎲 Nairobi is nicknamed the \"Green City in the Sun\" — and it's one of the few capitals with a national park inside city limits.",
    "🎲 The first computer bug was an actual literal bug — a moth stuck in a relay in 1947.",
]

DONT_PRESS_RESPONSES = [
    "🚨 SYSTEM ALERT: You pressed the button. The button is now judging you.",
    "👀 Nothing happened. But now the bot knows you're the curious type.",
    "🫡 Congratulations, you've unlocked... absolutely nothing. Achievement: Curiosity.",
    "🐸 A wild button press appeared! It was not very effective.",
    "🎭 Plot twist: this button does literally nothing. You were warned.",
]
...
def main_menu_content(user):
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("❌ DON'T PRESS THIS", callback_data="dont_press"),
        ],
    ]
...
def _track_user(context, chat_id):
    """Remembers this chat_id so /broadcast can reach it later. Stored in
    bot_data (shared, persisted), not user_data (per-user)."""
    known_users = context.application.bot_data.setdefault("known_users", set())
    known_users.add(chat_id)


async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _track_user(context, update.effective_chat.id)
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif query.data == "fun_fact":
        fact = random.choice(FUN_FACTS)
        keyboard = [[InlineKeyboardButton("🎲 ANOTHER ONE", callback_data="fun_fact")], START_BUTTON_ROW]
        await render(context, chat_id, fact, keyboard=keyboard, query=query)

    elif query.data == "dont_press":
        response = random.choice(DONT_PRESS_RESPONSES)
        keyboard = [[InlineKeyboardButton("❌ PRESS IT AGAIN", callback_data="dont_press")], START_BUTTON_ROW]
        await render(context, chat_id, response, keyboard=keyboard, query=query)
...
async def handle_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("payment_in_progress"):
        # A previous phone-number submission already kicked off an STK push
        # that's still being polled. Starting another here would fire a
        # second concurrent charge+poll for the same user — and whichever
        # one finishes last would overwrite the screen, even if it's the
        # stale one reporting a false failure over a real success.
        await render(
            context, chat_id,
            "⏳ A payment request is already in progress. Please approve "
            "the M-Pesa prompt you already received, or wait a moment "
            "before trying again.",
            dead_end=True,
        )
        return
    ...
    try:
        ...
        context.user_data["payment_in_progress"] = False
...
def get_live_max_price():
    """
    Asks HeroSMS for the current cheapest available price for our
    service/country, and returns that plus a small buffer as the maxPrice
    to send — so we track the real market instead of guessing a fixed
    number. Falls back to the static MAX_PRICE if the lookup fails or
    returns something we don't understand. Always capped at
    ABSOLUTE_MAX_PRICE regardless of what HeroSMS reports.
    """
    try:
        response = requests.get(
            HEROSMS_URL,
        ...
        # Expected shape: {"<country_id>": {"<service>": {"cost": 0.024, "count": 19135}}}
        service_data = data.get(str(COUNTRY), {}).get(SERVICE, {})
        current_price = float(service_data.get("cost"))

        target = current_price + PRICE_BUFFER
        return min(target, ABSOLUTE_MAX_PRICE)

    except Exception as e:
        print("getPrices lookup failed, falling back to static MAX_PRICE:", e)
        return min(MAX_PRICE, ABSOLUTE_MAX_PRICE)
...
async def purchase_number(context, chat_id, operator=None, require_payment=True, query=None, force_new_on_success=False):
    ...
    live_max_price = get_live_max_price()
    ...
    params = {
        ...
        "service": SERVICE,
        "country": COUNTRY,
        "maxPrice": live_max_price,
        # Do NOT send an operator for the normal request. HeroSMS can then
        # choose whichever operator currently has a number available at
        # or below our price ceiling. The operator sequence remains a
        # fallback for replacement/operator-specific requests.
    }
    if operator:
    ...
    try:
        ...
        print("HeroSMS getNumber request:", debug_params)
        ...
        if result.startswith("ACCESS_NUMBER:"):
            ...
            if len(parts) >= 3:
                ...
                sent = await render(
                    ...
                    "👉 *I'll watch for your code automatically* and send it "
                    "the moment it arrives — no need to keep checking. "
                    "(You can still tap CHECK STATUS anytime.) If nothing shows "
                    "up after 3.5 minutes, tap 🔴 HELP for a free replacement number.",
                    keyboard=keyboard,
                    parse_mode="Markdown",
                    query=query,
                ...
                _schedule_autocode_job(context, chat_id, activation_id)
        ...
        elif result.startswith("WRONG_MAX_PRICE:"):
            # HeroSMS is telling us that the current minimum price is above
            # even our live-buffered ceiling — an actual price spike, since
            # this ceiling is normally computed fresh from the live price.
            minimum = result.split(":", 1)[1].strip()
            await render(
                context, chat_id,
                "❌ No Viber Kenya number is currently available within "
                f"our price limit (we offered up to ${live_max_price:.3f}).\n\n"
                f"HeroSMS currently requires at least ${minimum}. "
                "Prices spiked right at this moment — try again shortly.",
                query=query,
                dead_end=True,
            )
...
async def get_number(context, chat_id, query=None):
    # Original purchase lets HeroSMS choose any operator with availability
    # under the price ceiling. The confirmation should land as a fresh bubble below the
    # phone number the user just typed, not edit the older screen above it.
    await purchase_number(
        context, chat_id, operator=None, require_payment=True,
...
async def handle_status_result(context, chat_id, result, query=None):
    ...
    if kind == "code":
        ...
        _cancel_autocode_job(context, chat_id)
        await render(
            context, chat_id,
            f"✅ Your code is: `{text}`",
            keyboard=number_nav_keyboard(context),
...
FUN_QUOTES = [
    "\"Numbers never lie — but they do expire in 20 minutes.\" 🎭",
    "\"In a world of permanent footprints, be a temporary number.\" 👣",
    "\"Ghosts don't need SIM cards. You barely do either.\" 👻",
    "\"With great uptime comes great WRONG_MAX_PRICE errors.\" 🕷️",
    "\"Behind every successful OTP is a bot that didn't sleep.\" 🤖",
]


async def _info_admin_dump(update: Update):
    """Original reference dump of HeroSMS's country and service lists.
    Now admin-only, via `/info admin`."""
    try:
        countries_response = requests.get(
            HEROSMS_URL,
            params={"api_key": HEROSMS_API_KEY, "action": "getCountries"},
            timeout=15,
        )
        services_response = requests.get(
            HEROSMS_URL,
            params={"api_key": HEROSMS_API_KEY, "action": "getServicesList"},
            timeout=15,
        )
        await update.message.reply_text("🌍 COUNTRIES:\n\n" + countries_response.text[:3500])
...
async def info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Fun, friendly info screen for everyone. The raw HeroSMS data dump is
    still available to the admin only, via `/info admin`."""
    args = context.args or []
    if (
        args
        and args[0].lower() == "admin"
        and ADMIN_ID
        and str(update.effective_user.id) == str(ADMIN_ID)
    ):
        await _info_admin_dump(update)
        return
    ...
    text = (
        "🤖 ABOUT THIS BOT\n\n"
        "Need a number for a quick verification code? You're in the right place. "
        "I hand out temporary numbers faster than you can say \"OTP\" ⚡\n\n"
        "✨ HOW IT WORKS\n"
        "1️⃣ Tap 🟣 GET NUMBER\n"
        "2️⃣ Pay with M-Pesa (STK push, no stress)\n"
        "3️⃣ Get your number and use it to receive your code\n"
        "4️⃣ Sit tight, I'll message you the moment your code arrives\n\n"
        f"💸 Price: just KES {PRICE_KES:.0f}\n"
        "⏱️ Your number lasts 20 minutes from purchase\n"
        "🔁 Code didn't arrive? Tap 🔴 HELP for a free replacement number\n\n"
        "🎉 BORED WHILE YOU WAIT?\n"
        "Try 🎲 FUN FACT below. Or press the ❌ button... we dare you 😏\n\n"
        + random.choice(FUN_QUOTES)
    )
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only. /broadcast <message> sends that text to every user who
    has ever /start-ed the bot. Silently does nothing for anyone else —
    the command isn't advertised in the menu, and non-admins get no reply,
    so it doesn't hint that a broadcast feature exists."""
    ...
    if not message:
        await update.message.reply_text(
            ...
            "It'll be sent as-is to every user who has started the bot."
        )
        return

    known_users = context.application.bot_data.setdefault("known_users", set())
    total = len(known_users)
    await update.message.reply_text(f"📣 Sending to {total} user(s)...")
    ...
    for uid in list(known_users):
        try:
            ...
            # User blocked the bot or deleted their account — stop trying them.
            known_users.discard(uid)
            blocked += 1
        except Exception:
            other_failures += 1
        # Telegram allows roughly 30 messages/sec across all chats — this
        # keeps us comfortably under that so large lists don't get rate-limited.
...
async def post_init(application):
    ...
    # Resume silent code-watching for numbers still active after a restart.
    try:
        now = time.time()
        for chat_id, data in application.user_data.items():
            ...
            if (
                activation_id and expiry_time and expiry_time > now
                and not data.get("code_received")
            ):
                _schedule_autocode_job_app(application, chat_id, activation_id)
    except Exception as e:
        print(f"[WARN] Could not resume code watchers: {e!r}")


class _HealthCheckHandler(BaseHTTPRequestHandler):
    """Bare-minimum HTTP responder. Its only job is to give Render's free
    Web Service something to point at — it has nothing to do with the bot's
    actual logic. An external pinger (e.g. UptimeRobot) hits this URL every
    few minutes so Render never sees 15 minutes of silence and puts the
    service to sleep."""
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    """Logs every unhandled error (including Telegram Conflict errors caused
    by a second running copy of the bot) so they show up in the deploy logs."""
    import traceback
...
def main():
    # PicklePersistence keeps bot_data (including known_users) on disk, so
    # the broadcast list survives a bot restart — not just in-memory.
    # NOTE: on Railway this file lives on the container's local disk, which
    # is wiped on redeploy unless you attach a persistent volume. Within a
    # single running deploy (crashes, restarts) it's safe.
    ...
    # Run the health-check web server on a background thread so it doesn't
    # block the bot's own polling loop below.
